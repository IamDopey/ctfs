...
Tried to run n nmap but its eems like its blocking all my requests

## Web

Like it was told in the challenge description what we haev here is an IDOR(insecure Direct Object Reference) vulnerability.

https://portswigger.net/web-security/access-control/idor

Looking at the page source code we can see that we have multiple doors and each door has a hash pointing to the room. We need to get a different hash that does not contain the image /static/img/empty_room.jpg

WE have the coordinates of each area:
...
Using https://www.dcode.fr/hash-function and pasting each hash we can see that each value is equal to a number using md5 algorithm, so we will build a program to create the hash for each value to 100 and spam it and see one that does not contain the image empty_room.jpg

After developing a script we found that one hash does not have empty_room.jpg image in it, cfcd208495d565ef66e7dff9f98764da.
